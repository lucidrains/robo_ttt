...
def test_fwpkm_memory_e2e():
    from fast_weight_product_key_memory import fwPKM
    from mimic_video import MimicVideo
    from robo_ttt import RoboTTT, MemoryKeyValueBind, fwPKMWrapper, TTTWrapper
    ...
    # 4 MLPs (index 0) and 1 fwPKM in the middle (index 1)
    ...
    pkm = fwPKM(dim = dim, num_memories = 16 * 16, dim_queries_keys = 16, dim_values = 16)
    fwpkm_memory = fwPKMWrapper(pkm)
    fwpkm_wrapper = TTTWrapper(dim, memory = fwpkm_memory, select_tokens_slice = slice(1, None))
    ...
    model = RoboTTT(
        policy,
        ttt_wrapper = (mlp_wrapper, fwpkm_wrapper),
    ...
    # 4 MLPs and 1 fwPKM in the middle (layer 2)
    assert 'memory_values' not in fast_weights[0].fast_weights
    assert 'memory_values' not in fast_weights[1].fast_weights
    assert 'memory_values' in fast_weights[2].fast_weights
    assert 'memory_values' not in fast_weights[3].fast_weights
    assert 'memory_values' not in fast_weights[4].fast_weights
